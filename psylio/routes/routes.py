...
def _endpoint_url(*segments, **query_params):
...
def login_url():
    return _endpoint_url(PSYLIO_URL, 'login')


def records_url(is_archived=False):
    return _endpoint_url(REQUEST_URL, 'archive' if is_archived else '')
...
def appointments_url(nb_days):
    ...
    return _endpoint_url(PSYLIO_URL, *segments, **query_params)


def open_invoices_url(nb_days):
    start, end = get_date_range(nb_days)
    query_params = dict(start=start, end=end, date_type='manual')
    return _endpoint_url(PSYLIO_URL, 'invoices', **query_params)


def record_url(record_id):
    return _endpoint_url(REQUEST_URL, record_id)


def profile_url(record_id):
    return _endpoint_url(record_url(record_id), 'profile')
...
def invoice_url(record_id, invoice_id=None, create=False):
    ...
    return _endpoint_url(record_url(record_id), 'invoices', suffix)
...
def record_invoices_url(record_id, state=None):
    ...
    return _endpoint_url(record_url(record_id), 'invoices', **query_params)


def invoice_create_url(record_id):
    return _endpoint_url(record_invoices_url(record_id), 'create')
